...
* `services` - Comma-separated list of service resources to export. If not specified, all resources within the given compartment (which excludes identity resources) are exported. The following values can be specified:
    ...
    * `database_tools_runtime` - Discovers database_tools_runtime resources within the specified compartment
    ...
    * `self` - Discovers self resources within the specified compartment
...
You can preserve the Terraform state file when a 404 Not Found or NotAuthorized response is encountered by setting TF_PRESERVE_STATE_ON_404. When set to true, the provider will not remove a resource from the Terraform state file if the OCI API returns a 404 Not Found or NotAuthorized response. By default, if a resource returns 404, it is removed from the state file.

```
export TF_PRESERVE_STATE_ON_404=<true|false>
```
...
* oci\_database\_tools\_runtime\_database\_tools\_database\_api\_gateway\_config\_pool\_api\_spec
* oci\_database\_tools\_runtime\_database\_tools\_database\_api\_gateway\_config\_pool
* oci\_database\_tools\_runtime\_database\_tools\_connection\_credential
* oci\_database\_tools\_runtime\_database\_tools\_database\_api\_gateway\_config\_pool\_auto\_api\_spec
* oci\_database\_tools\_runtime\_database\_tools\_connection\_credential\_execute\_grantee
